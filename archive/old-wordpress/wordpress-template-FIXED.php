<?php
/*
Template Name: AI Visa Eligibility Checker – ULTRA FAST + SMOOTH SCROLL (FINAL FIXED)
*/
get_header();
?>

<!-- CRITICAL PRELOADS + DNS PREFETCH -->
...
html{scroll-behavior:auto !important} /* SMOOTH SCROLL DISABLE */
body{font-family:'Inter',sans-serif;line-height:1.6;color:var(--dark);overflow-x:hidden;will-change:auto}
.container{max-width:1200px;margin:0 auto;padding:0 15px}

/* PROGRESS BAR - OPTIMIZED */
.progress-container{position:fixed;top:0;left:0;width:100%;height:4px;z-index:9999;background:var(--mint-light);overflow:hidden;will-change:transform}
.progress-bar{height:100%;background:var(--gradient);transform:translateX(-100%);transition:transform .1s ease-out;will-change:transform}

/* Language toggle */
.lang-switcher{
 position:fixed;top:120px;left:16px;z-index:1000;background:var(--glass);backdrop-filter:blur(20px);
 border:1px solid rgba(255,255,255,0.6);border-radius:25px;padding:6px;box-shadow:0 8px 32px rgba(0,0,0,0.12);will-change:auto;
}
.lang-btn{display:block;border:0;background:none;padding:8px 14px;font-weight:600;font-size:11px;cursor:pointer;border-radius:20px;
 transition:all .25s ease;color:var(--dark)!important;will-change:auto;}
.lang-btn.active{background:var(--gradient);color:#fff!important;box-shadow:0 4px 12px rgba(16,185,129,0.3);}

/* Hero */
.hero{position:relative;background:var(--gradient);color:#fff;text-align:center;overflow:hidden;padding:50px 18px 40px;will-change:auto}
...
/* TOOL BOX - FIXED BLUR + PERFORMANCE */
...
.tool-box{
 ...
 backdrop-filter:blur(10px);transition:all .4s ease;position:relative;will-change:auto;
}
.tool-box:hover{
 box-shadow:0 60px 120px rgba(16,185,129,0.35);
 border-color:var(--mint);
}

.tool-loading{
 position:absolute;top:0;left:0;right:0;bottom:0;background:var(--glass);
 display:flex;flex-direction:column;align-items:center;justify-content:center;
 z-index:10;backdrop-filter:blur(5px);border-radius:28px;will-change:auto;
 transition:opacity 0.3s ease;
}
.tool-loading.hidden{opacity:0;pointer-events:none;}
.loading-spinner{
 width:60px;height:60px;border:4px solid rgba(16,185,129,0.2);border-top:4px solid var(--mint);
 border-radius:50%;animation:spin 1s linear infinite;margin-bottom:20px;will-change:transform;
}
@keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}

.loading-text{font-weight:600;color:var(--dark);font-size:1.1rem;margin:0}
.loading-subtext{font-size:0.9rem;color:#64748b;margin:8px 0 0;opacity:0.9}

.tool-error{
 position:absolute;top:0;left:0;right:0;bottom:0;background:var(--glass);
 display:none;flex-direction:column;align-items:center;justify-content:center;
 z-index:10;backdrop-filter:blur(5px);border-radius:28px;color:var(--dark);will-change:auto;
}
.tool-error.show{display:flex}

/* IFRAME PERFECT - NO BLUR */
#visaIframe{
...
/* Sections & Cards - PERFORMANCE OPTIMIZED */
.section{padding:65px 0;will-change:auto}.section.alt{background:linear-gradient(180deg,var(--bg) 0%,rgba(16,185,129,0.02) 100%)}
...
.card{background:var(--glass);padding:35px;border-radius:24px;border:1px solid var(--mint-light);box-shadow:0 15px 40px rgba(16,185,129,0.1);
 transition:all .4s ease;backdrop-filter:blur(10px);position:relative;overflow:hidden;will-change:auto;}
...
.step-number{position:absolute;left:22px;top:50%;transform:translateY(-50%);width:44px;height:44px;border-radius:50%;
 ...
 box-shadow:0 10px 25px rgba(16,185,129,0.4);border:3px solid #fff;will-change:auto;}

/* Trust Badges */
.trust-badges{background:var(--gradient);color:#fff;padding:35px 0;text-align:center;position:relative;overflow:hidden;will-change:auto}
...
.trust-stats div{min-width:110px;text-align:center;line-height:1.2;position:relative;will-change:auto}
...
.hotline{font-size:1.1rem;font-weight:700;margin-top:15px;padding:12px 24px;background:var(--glass);border-radius:50px;
 display:inline-block;box-shadow:0 8px 25px rgba(0,0,0,0.15);border:2px solid rgba(255,255,255,0.3);transition:all .3s;color:#000000 !important;will-change:auto;}
...
.eligibility-cta{text-align:center;margin:50px 0;padding:35px 25px;background:linear-gradient(135deg,var(--gold),var(--orange));
 ...
 backdrop-filter:blur(10px);will-change:auto;}
.eligibility-btn{display:inline-flex;align-items:center;justify-content:center;gap:12px;padding:22px 48px;font-weight:800;font-size:16px;
 ...
 transition:all .5s ease;min-height:64px;letter-spacing:0.5px;position:relative;overflow:hidden;will-change:auto;}
...
.disclaimer-box{background:var(--glass);padding:30px 35px;border-radius:20px;border:1px solid var(--mint-light);text-align:center;
 margin:50px auto;max-width:950px;box-shadow:0 20px 50px rgba(16,185,129,0.08);backdrop-filter:blur(15px);will-change:auto;}
...
.layered { transform: translateZ(0); will-change: transform; }
</style>
...
<!-- ULTRA FAST + SMOOTH IFRAME TOOL -->
<div class="container tool-container">
 <div class="tool-box">
  <!-- Loading State - WILL AUTO HIDE AFTER 2 SECONDS -->
  <div class="tool-loading" id="toolLoading">
   <div class="loading-spinner"></div>
   <p class="loading-text">Loading Visa Checker...</p>
   <p class="loading-subtext">Please wait while we load the tool</p>
  </div>

  <!-- SIMPLE IFRAME - NO COMPLICATIONS -->
  <iframe
    id="visaIframe"
...
<!-- ALL ORIGINAL CONTENT SAME AS BEFORE -->
<div class="container">
